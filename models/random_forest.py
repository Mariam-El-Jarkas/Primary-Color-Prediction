...
for nl in noise_levels:
    ...
    train_acc = accuracy_score(y_train, dt_noisy.predict(X_train_noisy))
    ...
    if nl == 0:
        print(f"\n📌 Baseline (No Noise): Train Acc={train_acc:.4f}, Test Acc={test_acc:.4f}, F1={f1_noisy:.4f}, Fit={fit_status}")
    else:
        delta = test_acc - best_acc
        print(f"Noise {nl*100:.0f}%: Train Acc={train_acc:.4f}, Test Acc={test_acc:.4f} (Δ={delta:+.4f}), F1={f1_noisy:.4f}, Fit={fit_status}")
...
# Noise Impact
ax2.plot([nl*100 for nl in noise_levels], [noise_results[nl]['accuracy'] for nl in noise_levels], 'mo-', label='Test Acc')
